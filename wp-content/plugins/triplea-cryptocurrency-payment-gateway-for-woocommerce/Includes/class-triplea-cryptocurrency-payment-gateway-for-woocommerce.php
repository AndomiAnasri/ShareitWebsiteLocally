...
class TripleA_Cryptocurrency_Payment_Gateway_for_WooCommerce extends WPPB_Object {
	...
	protected function define_admin_hooks() {
        ...
        if ( get_option( 'triplea_dismiss_notice' ) != 'yes' ) :
            $this->loader->add_action( 'admin_notices', $plugins_page, 'general_admin_notice' );
        endif;

        $this->loader->add_action( 'admin_notices', $plugins_page, 'triplea_void_spare_me' );
        
        $installDate = get_option( 'triplea_crypto_plugin_activation_time' );
        $checkRated  = get_option( 'triplea_rated' );
        $showNoticeAfterThirtyDays = strtotime( "+30 days", $installDate );
        $showDate = strtotime("now");
        if ( $checkRated == 'yes' ) {
            return;
        } else {
            if ( $showDate >= $showNoticeAfterThirtyDays ) {
                $this->loader->add_action('admin_notices', $plugins_page, 'review_admin_notice');
            }
        }

      //$this->loader->add_filter( 'woocommerce_available_payment_gateways', TripleA_Payment_Gateway::class, 'triplea_exclude_if_subscription_product' );
	}
	...
	protected function define_woocommerce_hooks() {
        ...
        $this->loader->add_action( 'wp_ajax_triplea_orderpay_payment_request', TripleA_Payment_Gateway::class, 'triplea_orderpay_payment_request' );
        $this->loader->add_action( 'wp_ajax_nopriv_triplea_orderpay_payment_request', TripleA_Payment_Gateway::class, 'triplea_orderpay_payment_request' );
