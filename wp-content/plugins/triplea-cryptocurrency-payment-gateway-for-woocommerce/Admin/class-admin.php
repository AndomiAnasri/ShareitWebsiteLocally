...
class Admin {
	...
	public function print_wc_needed() {
		...
		echo '<div class="error"><p><strong>' . sprintf( esc_html__( 'TripleA Cryptocurrency Payment Gateway plugin requires WooCommerce to be installed and active. You can download %s here.', 'triplea-cryptocurrency-payment-gateway-for-woocommerce' ), '<a href="https://woocommerce.com/" target="_blank">WooCommerce</a>' ) . '</strong></p></div>';
	}
	...
	public function wc_admin_notices() {
		...
		add_filter(
			...
			function ( $translated_text, $untranslated_text, $domain ) {
				...
				$new = "<span style='color:red'>Cryptocurrency Payment Gateway for WooCommerce (by TripleA) </span> - Plugin needs WooCommerce to work.";
	...
	public function settings_update_notice() {
        ...
        if ((!empty($old_btc2fiat_api_id) || !empty($old_btc2btc_api_id)) && empty($fiat_merchant_key) && empty($btc_merchant_key) && empty($btc_sandbox_merchant_key)) {
            if ( $pagenow == 'admin.php' || $pagenow == 'plugins.php' || $pagenow == 'plugin-install.php' ) {
                ...
                $setting_text = __( 'Cryptocurrency payment settings', 'triplea-cryptocurrency-payment-gateway-for-woocommerce' );

                $message      = 'Your "Cryptocurrency Payment Gateway for WooCommerce" plugin has been disabled after the update. <br>
                            <b>Please take a short moment to update the settings</b> and re-enable cryptocurrency payments. &nbsp; &nbsp; <a href="' . $setting_link . '" target="_self">' . $setting_text . '</a>';
                $allowed_html = array(
                    'a' => array(
                        'href'   => array(),
                        'target' => array(),
