(function ($) {
    ...
    window.triplea_validateCheckout = function (elem) {
        ...
        if(elem.classList.contains('triplea-order-pay')){
            ...
            let url = triplea_object.ajax_url + '?action=triplea_orderpay_payment_request';
            let data = 'orderid=' + elem.dataset.id;
            //triplea_ajax_action(url, orderpay_checkout, "GET", null, false);
            ...
            if ( xmlHttp.responseText != null ) {
                ...
                hiddenInput = document.createElement("input");
                hiddenInput.setAttribute("id", 'triplea_embedded_payment_form_url');
                hiddenInput.setAttribute("name", 'triplea_embedded_payment_form_url');
                hiddenInput.setAttribute("type", "hidden");
    ...
    function paymentMethodAction() {
        if ($('form[name="checkout"] input[name="payment_method"]:checked').val() === 'triplea_payment_gateway'){
            $('#place_order').css('opacity', 0.25);
            $('#place_order').css('visibility', 'hidden');
        } else if ($('#order_review input[name="payment_method"]:checked').val() === 'triplea_payment_gateway'){
            $('#place_order').css('opacity', 0.25);
            $('#place_order').css('visibility', 'hidden');
        } else{
            $('#place_order').css('opacity', 1.0);
    ...
    $(document).ready(function (e) {
        ...
        if( $('#order_review input[name="payment_method"]:checked').val() == 'triplea_payment_gateway' ) {
            paymentMethodAction();
        }
