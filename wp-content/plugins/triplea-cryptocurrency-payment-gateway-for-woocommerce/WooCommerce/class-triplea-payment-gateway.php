...
class TripleA_Payment_Gateway extends WC_Payment_Gateway {
    ...
    */
    ...
    public function __construct() {
        ...
        $this->title        = ''.__('Cryptocurrency Payment Gateway', 'triplea-cryptocurrency-payment-gateway-for-woocommerce').'';
        $this->method_title = ''.__('Cryptocurrency Payment Gateway', 'triplea-cryptocurrency-payment-gateway-for-woocommerce').'';
        $this->has_fields   = TRUE;
        $this->supports     = [
            'products',
        ];

        //$this->order_button_text = 'Pay with cryptocurrency';
        $this->order_button_text = 'Waiting for payment';

        /**
         * Display text customisation options
         */
        $this->description = __('Secure and easy payment with Cryptocurrency using the Triple-A.io service.', 'triplea-cryptocurrency-payment-gateway-for-woocommerce');
        $this->init_form_fields();
        $this->init_settings();

        $debug_log_enabled  = $this->get_option('debug_log_enabled') === 'yes';
        $support_log_keeper = $this->get_option('admin_debug_log_enabled') === 'yes' ? TRUE : FALSE;
        ...
        $this->triplea_active_api_id_auth_reset = $this->get_option('triplea_active_api_id_auth_reset');
        ...
        if (isset($this->triplea_active_api_id) && !empty($this->triplea_active_api_id) && isset($this->triplea_active_api_id_auth_reset) && $this->triplea_active_api_id_auth_reset === 'true') {
            triplea_write_log('Account replaced. Requesting fresh oauth token for new account.', $debug_log_enabled);
            $this->update_option('triplea_active_api_id_auth_reset', 'false');
            $this->refreshOauthTokens($this->triplea_active_api_id);
        }
        ...
        $this->method_description = sprintf(
            __(
                '%1$s With %2$s you get to choose how your cryptocurrency payments are settled to you: %3$s or %4$s.',
                'triplea-cryptocurrency-payment-gateway-for-woocommerce'
            ),
            '<img src="https://triple-a.io/wp-content/uploads/2020/08/TripleA_MainLogo_Transparent-e1596423970124.png" alt="Footer Logo" class="webpexpress-processed" style="max-width: 149px; vertical-align: text-bottom;display: inline-block;position: relative;top: 5px;margin-right: 15px;">',
            '<a href="https://triple-a.io">TripleA</a>',
            '<strong>'.__('in cryptocurrency', 'triplea-cryptocurrency-payment-gateway-for-woocommerce').'</strong>',
            '<strong>'.__('in your local currency', 'triplea-cryptocurrency-payment-gateway-for-woocommerce').'</strong>'
        );
    ...
    public static function update_order_status($payment_data, $wc_order, $placing_new_order = false, $unix_timestamp = null, $hex_signature = null) {
        ...
        $support_log_keeper = $triplea->get_option('admin_debug_log_enabled') === 'yes' ? TRUE : FALSE;
        ...
        if ('wc-'.$order_status === $order_status_invalid || $order_status === 'failed' || strpos($order_status,'fail') !== FALSE || strpos($order_status,'invalid') !== FALSE) {
            ...
            triplea_write_log("update_order_status() : order status is ".$order_status.". The following information is added for logging purposes, the order status will not be updated. Contact us at plugin.support@triple-a.io if you think there has been a mistake.", $debug_log_enabled);
        }
    ...
    public function payment_scripts() {
        ...
        wp_localize_script( 'triplea_payment_gateway_embedded_payment_form_js', 'triplea_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
    }
    ...
    public static function triplea_exclude_if_subscription_product( $available_gateways ) {
        ...
        if ( ! class_exists('WC_Subscriptions') ) return $available_gateways;
        if ( ! class_exists('WC_Subscriptions_Cart') ) return $available_gateways;
...
//   }
    ...
    public function triplea_cryptocurrency_payment_gateway_for_woocommerce_notice_payments_enabled() {
        echo '<div class="updated notice is-dismissable"><p>' . __('You are now accepting cryptocurrency payments.', 'triplea-cryptocurrency-payment-gateway-for-woocommerce') . '</p></div>';
    }

    public function triplea_cryptocurrency_payment_gateway_for_woocommerce_notice_payments_disabled() {
        echo '<div class="updated notice is-dismissable"><p>' . __('Cryptocurrency Payments disabled.', 'triplea-cryptocurrency-payment-gateway-for-woocommerce') . '</p></div>';
    }

    public function triplea_cryptocurrency_payment_gateway_for_woocommerce_notice_payments_disabled_missing_apiid() {
        echo '<div class="error notice is-dismissable"><p>' . __('Cryptocurrency payments disabled, no active wallets.', 'triplea-cryptocurrency-payment-gateway-for-woocommerce') . '</p></div>';
    }
    ...
    public function get_title() {
        if ('custom-text' === $this->get_option('triplea_bitcoin_text_option')) {
            ...
            $title = __('Cryptocurrency', 'triplea-cryptocurrency-payment-gateway-for-woocommerce');
        }
    ...
    public function get_description() {
        if ('desc-default' === $this->get_option('triplea_bitcoin_descriptiontext_option') || empty($this->get_option('triplea_bitcoin_descriptiontext_option'))) {
            $description = __('Secure and easy payment with Cryptocurrency', 'triplea-cryptocurrency-payment-gateway-for-woocommerce');
    ...
    public function get_icon() {
      ...
      $logo = $this->get_option('triplea_bitcoin_logo_option');
        ...
        switch ($logo) {
            ...
            case 'eth-large-logo':
                $iconfile = 'eth-full.png';
                $style    = 'style="max-width: 100px !important;max-height: 30px !important;"';
                break;

            case 'eth-short-logo':
                $iconfile = 'eth-icon.png';
                $style    = 'style="max-width: 100px !important;max-height: 30px !important;"';
                break;

            case 'usdt-large-logo':
                $iconfile = 'tether-full.png';
                $style    = 'style="max-width: 100px !important;max-height: 30px !important;"';
                break;

            case 'usdt-short-logo':
                $iconfile = 'tether-icon.png';
                $style    = 'style="max-width: 100px !important;max-height: 30px !important;"';
                break;

            case 'default-large-logo':
                $iconfile = 'crypto-default-full.png';
                $style    = 'style="max-width: 50% !important;max-height: 30px !important;"';
                break;

            case 'default-short-logo':
                $iconfile = 'crypto-default-icon.png';
                $style    = 'style="max-width: 50% !important;max-height: 30px !important;"';
                break;
    ...
    public function process_payment($order_id) {
        ...
        if(is_wc_endpoint_url( 'order-pay' )) {
            ...
            if ( $support_log_keeper ) :
            triplea_write_log("Order pay page process_payment() : payment status check, received data : \n" . print_r($payment_data, TRUE), $debug_log_enabled);
            endif;
            $status_info = null;
            if ( !isset($payment_data->error) ) {
        ...
        if ($payment_mode != 'bitcoin-to-cash') {
        ...
        if ($cart_total_paid_str != $order_total_due_str) {
            $CART_TAMPERED = TRUE;
            triplea_write_log('process_payment() : shopping cart value during cryptocurrency payment: '.$cart_total_paid_str, $debug_log_enabled);
            triplea_write_log('process_payment() : shopping cart value after order placement : '.$order_total_due_str, $debug_log_enabled);
            triplea_write_log('process_payment() : WARNING! Shopping cart contents were modified between the moment the cryptocurrency payment form was displayed and the moment the cryptocurrency payment was made.'.$order_total_due_str, $debug_log_enabled);
   ...
   return ob_get_clean();
    ...
    public function display_embedded_payment_form_button( $button_html ) {
        ...
        $order_button_text = __('Pay with Cryptocurrency', 'triplea-cryptocurrency-payment-gateway-for-woocommerce');
        $order_button_desc = sprintf(__('Please pay the %1$s. Avoid paying from a crypto exchange, use your personal wallet.', 'triplea-cryptocurrency-payment-gateway-for-woocommerce'),
                            '<strong>'.__('exact amount', 'triplea-cryptocurrency-payment-gateway-for-woocommerce').'</strong>'
                           );
        ...
        if( $this->get_option('triplea_payment_mode') == 'bitcoin-to-bitcoin' )
            return 'Please switch to Local currency settlements to continue the checkout! If you want a custody account option please contact our support team via email to <a href="mailto:support@triple-a.io">support@triple-a.io</a>';
        else
    ...
    public static function triplea_ajax_get_payment_form_data() {
        ...
        $triplea           = new TripleA_Payment_Gateway();
        $debug_log_enabled  = $triplea->get_option('debug_log_enabled') === 'yes';
        $support_log_keeper = $triplea->get_option('admin_debug_log_enabled') === 'yes';
    ...
    private function refreshOauthTokens($refresh_for_api_id = FALSE) {
        $debug_log_enabled  = $this->get_option('debug_log_enabled') === 'yes';
        $support_log_keeper = $this->get_option('admin_debug_log_enabled') === 'yes';
        ...
        if (isset($this->triplea_btc2fiat_client_id) && !empty($this->triplea_btc2fiat_client_id)
            ...
            && (!isset($this->triplea_btc2fiat_oauth_token) || empty($this->triplea_btc2fiat_oauth_token) || $this->triplea_btc2fiat_oauth_token_expiry <= $date_expiry_limit || $this->triplea_btc2fiat_api_id === $refresh_for_api_id)
        ) {
            ...
            elseif ($this->triplea_btc2fiat_api_id === $refresh_for_api_id) {
                triplea_write_log('refreshOauthToken() OAuth token (for local currency settlement) reset due to new account init. Requesting a new oauth token.', $debug_log_enabled);
            }
            else {
        ...
        if (isset($this->triplea_btc2btc_client_id) && !empty($this->triplea_btc2btc_client_id)
            ...
            && (!isset($this->triplea_btc2btc_oauth_token)
                ...
                ||  $this->triplea_btc2btc_api_id === $refresh_for_api_id
        )) {
            ...
            } elseif ($this->triplea_btc2btc_api_id === $refresh_for_api_id) {
                triplea_write_log('refreshOauthToken() OAuth token (for live btc settlement) reset due to new account init. Requesting a new oauth token.', $debug_log_enabled);
            } else {
        ...
        if ($this->triplea_btc2btc_sandbox_client_id === $this->triplea_btc2btc_client_id && $this->triplea_btc2btc_sandbox_client_secret === $this->triplea_btc2btc_client_secret) {
          ...
          && (!isset($this->triplea_btc2btc_sandbox_oauth_token)
              ...
              || $this->triplea_btc2btc_sandbox_api_id === $refresh_for_api_id)
        ) {
            ...
            } elseif ($this->triplea_btc2btc_sandbox_api_id === $refresh_for_api_id) {
                triplea_write_log('refreshOauthToken() OAuth token (for sandbox btc settlement) reset due to new account init. Requesting a new oauth token.', $debug_log_enabled);
            } else {
    ...
    private function get_payment_form_request(
        ...
        if ($payment_mode === 'bitcoin-to-bitcoin' && !$sandbox_payment_mode) {
            ...
            if (empty($oauth_token)) {
                wp_die('Missing oauth token for live cryptocurrency payments to cryptocurrency wallet.');
            }
        }
        elseif ($payment_mode === 'bitcoin-to-bitcoin' && $sandbox_payment_mode) {
            ...
            if (empty($oauth_token)) {
                wp_die('Missing oauth token for sandbox cryptocurrency payments to cryptocurrency wallet.');
            }
        }
        else {
    ...
    * DEPRECATED "unconfirmed_crypto_amt": 0,
    * DEPRECATED "unconfirmed_order_amt": 0,
    * DEPRECATED "confirmed_crypto_amt": 0.00023,
    * DEPRECATED "confirmed_order_amt": 2.3,
    * "payment_crypto_amount": 2.3,
    * "status": "confirmed",
    ...
    */
    ...
    private function preparePaymentFormRequestBody(
        ...
        if ($order_id != null) {
            ...
            triplea_write_log('Order Pay Page Cart Total: '.$order->get_total(), $debug_log_enabled);
            ...
            triplea_write_log('Order Pay Page Check: Nah '.esc_attr(WC()->cart->total), $debug_log_enabled);
        }
        ...
        if( $payment_mode === 'bitcoin-to-cash' ) {
            $body += [ "merchant_key" => $this->triplea_btc2fiat_merchant_key, "sandbox" => ($sandbox_payment_mode) ? 'true' : 'false' ];
        } else {
            $body += [
                "api_id"          => $api_id,
